...
function CustomProfileSet:SetupHooks()
    if self:IsInit() then
        ...
        elseif RequiredScript == "lib/managers/menu/renderers/menunodeskillswitchgui" then
            Hooks:PostHook(
                ...
                "_create_menu_item",
                "CPS_SkillSwitchGui_CreateMenuItem",
                function(_, item)
                    local gui = item.skill_points_gui
                    if
                        gui and
                            gui:text() ==
                                managers.localization:to_upper_text("menu_st_requires_skill_switch", {reasons = ""})
                     then
            ...
            Hooks:PostHook(
                MenuNodeSkillSwitchGui,
                "mouse_pressed",
                "CPS_SkillSwitchGui_MousePressed",
                function(gui, button, x, y)
                ...
                if skill_set and skill_set.unlocked then
                    ...
                    local menu =
                        ...
                        managers.localization:text(
                            this.skill_menu_name,
                            {name = managers.skilltree:get_skill_switch_name(index, true)}
                        ),
                        managers.localization:text(
                            this.skill_menu_description,
                            {name = managers.localization:text(tweak_data.skilltree.specializations[perk_deck].name_id)}
                        ),
                        {
                            {
        ...
        elseif RequiredScript == "lib/managers/menu/multiprofileitemgui" then
            Hooks:PreHook(
                MultiProfileItemGui,
                "init",
                "CPS_ProfileItemGui_Pre_Init",
                function(td, ws, panel)
    ...
    Hooks:Add(
        ...
        "CPS_LocalizationManager_PostInit",
        function(manager)
    ...
    Hooks:Add(
        ...
        "CPS_MenuManager_PopulateCustomMenus",
        function()
    ...
    Hooks:Add(
        ...
        "CPS_MenuManager_BuildCustomMenus",
        function(_, nodes)
